...
bool CBot::NeedPickup(int Type)
{
	...
	case CTarget::TARGET_WEAPON_SHOTGUN:
		return m_pPlayer->GetCharacter()->GetAmmoCount(WEAPON_SHOTGUN) < 2;
	case CTarget::TARGET_WEAPON_GRENADE:
		return m_pPlayer->GetCharacter()->GetAmmoCount(WEAPON_GRENADE) < 3;
	case CTarget::TARGET_WEAPON_LASER:
		return m_pPlayer->GetCharacter()->GetAmmoCount(WEAPON_RIFLE) < 3;
	}
	return false;
}
...
void CBot::MakeChoice(bool UseTarget)
{
	...
	TempChar.m_Input.m_Direction = (m_Target.x > 28.f) ? 1 : (m_Target.x < -28.f) ? -1
																				  : 0;
